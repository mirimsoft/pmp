...
namespace Mirim
{
	BlowFish::BlowFish()
	{
		...
		BYTE key[] = "BlowFishKey"; 
		this->Initialize(key, sizeof(key) - 1);
	}
	...
	DWORD BlowFish::Encode(BYTE * pInput, BYTE * pOutput, DWORD lSize)
	{
		...
		for(lCount = 0; lCount < lOutSize; lCount += 8)
		{
			if (SameDest)	// if encoded data is being written into input buffer
			{
				...
				{
					this->Encipher((DWORD *) pInput, (DWORD *)(pInput + 4));
				}
			...
			{               // input to output buffer prior to encrypting
				if (lCount < lSize - 7)	// if not dealing with uneven bytes at end
				{
					...
					this->Encipher((DWORD *) pOutput, (DWORD *)(pOutput + 4)); // now encrypt them
				}
				else		// pad end of data with null bytes to complete encryption
				{
					...
					this->Encipher((DWORD *) pOutput, (DWORD *)(pOutput + 4));
				}
				pInput += 8;
				pOutput += 8;
			}
	...
	void BlowFish::Decode(BYTE * pInput, BYTE * pOutput, DWORD lSize)
	{
		...
		for(lCount = 0; lCount < lSize; lCount += 8)
		{
			if (SameDest)	// if encoded data is being written into input buffer
			{
				this->Decipher((DWORD *) pInput, (DWORD *)(pInput + 4));
				pInput += 8;
			}
			...
			{               // so copy input to output before decoding
				...
				for(i = 0; i < 8; i++)
					*po++ = *pi++;
				this->Decipher((DWORD *) pOutput, (DWORD *)(pOutput + 4));
				pInput += 8;
				pOutput += 8;
			}
		}
