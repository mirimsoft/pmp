    ...
    ProAppData dummy)
{
    ...
    /* internal datum plane found - create elemtree and check constarins*/
    for (i=0; i<2; i++)
    {
    ...
    ProAppData	appdata)
{
	...
	if (is_multi)
	{
	    ...
	    for (j=0; j<n_val; j++)
	    {
		err = ProUtilValueReplaceSelection(p_values + j, user_func, 
		    appdata);
		if (err == PRO_TK_NO_ERROR)
		    replace = PRO_B_TRUE;
	    }
	    ...
	    {
		err = ProElementValuesSet(p_elems[i].p_element, p_values, 
		    n_val);
		TEST_CALL_REPORT("ProElementValuesSet()",
		    "ProUtilElemtreeReplaceSelection()", 
	    		    			err, err != PRO_TK_NO_ERROR);
	    }
	    err = ProArrayFree((ProArray*)&p_values);
